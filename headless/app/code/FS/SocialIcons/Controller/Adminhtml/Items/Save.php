...
class Save extends \FS\SocialIcons\Controller\Adminhtml\Items
{
    public function execute()
    {
        if ($this->getRequest()->getPostValue()) {
            try {
                $model = $this->_objectManager->create('FS\SocialIcons\Model\SocialIcons');
                ...
                if ($this->getRequest()->getParam('back')) {
                    $this->_redirect('fs_socialicons/*/edit', ['id' => $model->getId()]);
                    return;
                }
                $this->_redirect('fs_socialicons/*/');
                return;
            } catch (\Magento\Framework\Exception\LocalizedException $e) {
                $this->messageManager->addError($e->getMessage());
                $id = (int)$this->getRequest()->getParam('id');
                if (!empty($id)) {
                    $this->_redirect('fs_socialicons/*/edit', ['id' => $id]);
                } else {
                    $this->_redirect('fs_socialicons/*/new');
                }
                return;
            } catch (\Exception $e) {
                ...
                $this->_redirect('fs_socialicons/*/edit', ['id' => $this->getRequest()->getParam('id')]);
                return;
            }
        }
        $this->_redirect('fs_socialicons/*/');
